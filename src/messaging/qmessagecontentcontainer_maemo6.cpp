...
{
    QString newId = QString::number(_attachments.count()+1);
    newId += "_";
    newId += container.d_ptr->_attachmentId;

    container.d_ptr->_id = QMessageContentContainerId(newId);
...
QMessageContentContainerId QMessageContentContainerPrivate::prependContent(QMessageContentContainer& container)
{
	...
	QString newId = QString::number(i+1);
	newId += "_";
	newId += container.d_ptr->_attachmentId;
        _attachments[i].d_ptr->_id = QMessageContentContainerId(newId);
    }
