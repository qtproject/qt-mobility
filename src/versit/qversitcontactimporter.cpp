...
/*!
  ...
  vCard properties not supported by QVersitContactImporter.
...
/*!
  ...
  and/or handle properties that QVersitContactImporter doesn't support.  By default, this is set to
  a handler that decodes properties generated by the QVersitContactExporter's default handler (see
  \l QVersitContactImporterPropertyHandlerV2::createDefaultHandler()).  This means that if a contact
  is exported and then imported again, the resulting contact will be virtually identical to the
  original one, even if there are non-standard details.
...
/*!
  \fn QVersitContactImporterPropertyHandlerV2* createDefaultHandler()
  Constructs a default property handler and returns it.  The caller is responsible for deleting the
  object.  If a QVersitContactImporter has not been explicitly associated with a property handler,
  it is associated with one created using this function.

  This handler decodes properties generated by the default exporter detail handler (created by
  QVersitContactExporterDetailHandlerV2::createDefaultHandler() ).
  ...
  QVersitContactExporterDetailHandlerV2 around it.  In the implementation of propertyProcessed and
  documentProcessed, the respective functions in the default handler should be called as the last
  step.
 */
QVersitContactImporterPropertyHandlerV2* QVersitContactImporterPropertyHandlerV2::createDefaultHandler() {
    return new QVersitContactImporterDefaultPropertyHandler;
}
