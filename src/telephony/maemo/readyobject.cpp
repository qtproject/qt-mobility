...
namespace DBus
{
    ...
    {
        if (features.isEmpty()) {
            return readinesshelper->isReady(Features() << featureCore);
        }
        return readinesshelper->isReady(features);
    }

    PendingReady *ReadyObject::becomeReady(const Features &requestedFeatures)
    {
        if (requestedFeatures.isEmpty()) {
            return readinesshelper->becomeReady(Features() << featureCore);
        }
        return readinesshelper->becomeReady(requestedFeatures);

    }

    Features ReadyObject::requestedFeatures() const
    {
        return readinesshelper->requestedFeatures();
    }

    Features ReadyObject::actualFeatures() const
    {
        return readinesshelper->actualFeatures();
    }

    Features ReadyObject::missingFeatures() const
    {
        return readinesshelper->missingFeatures();
    }
