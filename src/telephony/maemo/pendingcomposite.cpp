...
#include "maemo/pendingcomposite.h"
...
namespace DBus
{
    ...
    void PendingComposite::onOperationFinished(DBus::PendingOperation *op)
    {
        if (op->isError()) {
        ...
        if (++nOperationsFinished == nOperations) {
            setFinished();
        }
    }
} // DBus
