/****************************************************************************
**
** Copyright (C) 2010 Nokia Corporation and/or its subsidiary(-ies).
** All rights reserved.
** Contact: Nokia Corporation (qt-info@nokia.com)
**
** This file is part of the Qt Mobility Components.
**
** $QT_BEGIN_LICENSE:LGPL$
** No Commercial Usage
** This file contains pre-release code and may not be distributed.
** You may use this file in accordance with the terms and conditions
** contained in the Technology Preview License Agreement accompanying
** this package.
**
** GNU Lesser General Public License Usage
** Alternatively, this file may be used under the terms of the GNU Lesser
** General Public License version 2.1 as published by the Free Software
** Foundation and appearing in the file LICENSE.LGPL included in the
** packaging of this file.  Please review the following information to
** ensure the GNU Lesser General Public License version 2.1 requirements
** will be met: http://www.gnu.org/licenses/old-licenses/lgpl-2.1.html.
**
** In addition, as a special exception, Nokia gives you certain additional
** rights.  These rights are described in the Nokia Qt LGPL Exception
** version 1.1, included in the file LGPL_EXCEPTION.txt in this package.
**
** If you have questions regarding the use of this file, please contact
** Nokia at qt-info@nokia.com.
**
**
**
**
**
**
**
**
** $QT_END_LICENSE$
**
****************************************************************************/

#include "qlandmarkfetchhint.h"
#include "qlandmarkfetchhint_p.h"

QTM_BEGIN_NAMESPACE

/*!
  \class QLandmarkFetchHint
  \ingroup landmarks-main
  \brief The QLandmarkFetchHint class provides miscellaneous hints to the manager
  when fetching landmarks.

   The QLandmarkFetchHint class may be used to specify the maximum number
   of landmark items to return during a query and an offset to facilitate
   paging.
*/

/*!
    Constructs a new landmark fetch hint
*/
QLandmarkFetchHint::QLandmarkFetchHint()
    : d(new QLandmarkFetchHintPrivate)
{
}

/*!
    Constructs a new landmark fetch hint as a copy of \a other.
*/
QLandmarkFetchHint::QLandmarkFetchHint(const QLandmarkFetchHint &other)
    : d(other.d)
{
}

/*!
    Destroys the fetch hint
*/
QLandmarkFetchHint::~QLandmarkFetchHint()
{
}

/*!
    Assigns \a other to this fetch hint and returns a reference to this fetch hint.
*/
QLandmarkFetchHint &QLandmarkFetchHint::operator=(const QLandmarkFetchHint &other)
{
    d = other.d;
    return *this;
}

/*!
    Returns the maxmimum number of items to be retrieved.
*/
int QLandmarkFetchHint::maxItems()
{
    return d->maxItems;
}

/*!
    Sets the \a max number of items to be retrieved.
*/
void QLandmarkFetchHint::setMaxItems(int max)
{
    d->maxItems = max;
}

/*!
    Returns the offset of the items to be retrieved.
*/
int QLandmarkFetchHint::offset()
{
    return d->offset;
}

/*!
    Sets the \a offset for the items to be retrieved.
*/
void QLandmarkFetchHint::setOffset(int offset)
{
    d->offset = offset;
}

QTM_END_NAMESPACE
