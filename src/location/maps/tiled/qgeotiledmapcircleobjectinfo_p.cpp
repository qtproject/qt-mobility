...
void QGeoTiledMapCircleObjectInfo::objectUpdate()
{
    ...
    */
    ...
    //makepoly(points, path, mapData, true, mapData->q_ptr->coordinateToWorldPixel(center).y() + (center.latitude() > 0 ? 50 : -50)); // 50px towards the closest pole
    //makepoly(points, path, mapData, true); // always north pole
