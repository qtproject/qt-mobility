...
public:
    ...
    HRESULT STDMETHODCALLTYPE PreDeleteItem(
            DWORD dwFlags, IShellItem *psiItem);
    HRESULT STDMETHODCALLTYPE PostDeleteItem(
            DWORD dwFlags, IShellItem *psiItem, HRESULT hrDelete, IShellItem *psiNewlyCreated);
    HRESULT STDMETHODCALLTYPE PreNewItem(
            DWORD dwFlags, IShellItem *psiDestinationFolder, LPCWSTR pszNewName);
...
protected:
    virtual int initialize(IFileOperation *operation) = 0;

    IShellItem *createShellItem(const QString &id) const;
...
private:
    ...
    void run();
    bool setCurrentItem(IShellItem *item);
    bool clearCurrentItem();
    ...
    QList<int> m_listKeys;
    QVector<PROPERTYKEY> m_propertyKeys;
    QMutex m_mutex;
    Item m_current;
    Item m_pending;
