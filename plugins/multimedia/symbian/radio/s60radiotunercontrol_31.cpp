...
S60RadioTunerControl::~S60RadioTunerControl()
{
	if (m_tunerUtility) {
		...
		m_tunerUtility->ReleaseTunerControl();
...
bool S60RadioTunerControl::initRadio()
{
	...
	/*if (QFile::exists("z:/sys/bin/FmRadioEngine.dll")) {
		QMessageBox msgBox;
		msgBox.setText(tr("FmRadioEngine.dll"));
		msgBox.exec();
		}*/
...
void S60RadioTunerControl::MssoSignalStrengthChanged(TInt aNewSignalStrength)
{
	...
	QMessageBox msgBox;
	msgBox.setText(tr("MssoSignalStrengthChanged"));
	msgBox.exec();
}
