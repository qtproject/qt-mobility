...
QString LandmarkManagerEngineSymbianPrivate::managerUri() const
    {
    return QLandmarkManager::buildUri(managerName(), managerParameters(),
            managerVersion());
    }
 ...
 */
QList<QLandmarkId> LandmarkManagerEngineSymbianPrivate::landmarkIds(
        const QLandmarkFilter &filter, int limit, int offset, const QList<
    ...
    switch (filter.type())
        ...
        case QLandmarkFilter::IntersectionFilter:
            ...
            if (filters.size() == 0)
                ...
                {
                result = landmarkIds(filters.at(0), limit, offset, QList<
                ...
                QList<QLandmarkId> firstResult = landmarkIds(filters.at(0),
                        limit, offset, QList<QLandmarkSortOrder> (), error,
                        errorString);
                ...
                for (int i = 1; i < filters.size(); ++i)
                    ...
                    QList<QLandmarkId> subResult = landmarkIds(filters.at(i),
                            limit, offset, QList<QLandmarkSortOrder> (),
                            error, errorString);
 ...
 */
QList<QLandmarkCategoryId> LandmarkManagerEngineSymbianPrivate::categoryIds(
        int limit, int offset, const QLandmarkNameSort &nameSort,
        QLandmarkManager::Error *error, QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
 ...
 */
QLandmark LandmarkManagerEngineSymbianPrivate::landmark(
        const QLandmarkId &landmarkId, QLandmarkManager::Error *error,
 ...
 * 
 */
QList<QLandmark> LandmarkManagerEngineSymbianPrivate::landmarks(const QList<
...
QList<QLandmark> LandmarkManagerEngineSymbianPrivate::landmarks(
    ...
    *errorString = "";
    ...
    QList<QLandmarkId> ids = landmarkIds(filter, limit, offset, sortOrders,
            error, errorString);
    if (*error != QLandmarkManager::NoError)
        {
        return result;
    ...
    foreach(const QLandmarkId &id, ids)
            ...
            if (lm.landmarkId().isValid() && *error
...
/*!
 ...
 Overall operation errors are stored in \a error and \a errorString.
 */
QList<QLandmarkCategory> LandmarkManagerEngineSymbianPrivate::categories(
        const QList<QLandmarkCategoryId> &landmarkCategoryIds, QMap<int,
                QLandmarkManager::Error> *errorMap,
        QLandmarkManager::Error *error, QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
    ...
    *errorString = "";
    ...
    for (int i = 0; i < landmarkCategoryIds.size(); ++i)
        {
        QLandmarkCategory cat = category(landmarkCategoryIds.operator[](i),
...
QList<QLandmarkCategory> LandmarkManagerEngineSymbianPrivate::categories(
    ...
    *errorString = "";
    ...
    QList<QLandmarkCategoryId> qtCatIds;
    TRAPD(err, qtCatIds = fetchCategoryIdsL(nameSort, error, errorString);)
    if (err == KErrNone && *error == QLandmarkManager::NoError
            && qtCatIds.size() > 0)
        {
...
bool LandmarkManagerEngineSymbianPrivate::saveLandmark(QLandmark* landmark,
    ...
    *errorString = "";
    ...
    if (added)
        ...
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkAdded, landmarkIds);
        }
    if (changed)
        {
        landmarkIds.append(landmark->landmarkId());
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkUpdated, landmarkIds);
        }
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::saveLandmarks(
    ...
    *errorString = "";
    ...
    if (addedIds.size() > 0)
        {
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkAdded, addedIds);
        }
    if (changedIds.size() > 0)
        {
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkUpdated, changedIds);
        }
...
/*!
 ...
 \a error and \a errorString.
 */
bool LandmarkManagerEngineSymbianPrivate::removeLandmark(
        const QLandmarkId &landmarkId, QLandmarkManager::Error *error,
        QString *errorString)
    {
    Q_ASSERT(error);
    ...
    *errorString = "";
    ...
    if (removed)
        ...
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkRemoved, landmarkIds);
        }
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::removeLandmarks(const QList<
        QLandmarkId> &landmarkIds,
        QMap<int, QLandmarkManager::Error> *errorMap,
        QLandmarkManager::Error *error, QString *errorString)
    {
    ...
    *errorString = "";
    ...
    if (removedIds.size() > 0)
        {
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkRemoved, removedIds);
        }
...
/*!
 ...
 \a errorString.
 */
bool LandmarkManagerEngineSymbianPrivate::saveCategory(
        QLandmarkCategory* category, QLandmarkManager::Error *error,
        QString *errorString)
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
    ...
    *errorString = "";
    ...
    if (added)
        ...
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::categoryAdded, QList<QLandmarkId> (),
                categoryIds);
        }
    if (changed)
        {
        categoryIds.append(category->categoryId());
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::categoryUpdated,
                QList<QLandmarkId> (), categoryIds);
        }
...
/*!
 ...
 \a errorString.
 */
bool LandmarkManagerEngineSymbianPrivate::removeCategory(
        const QLandmarkCategoryId &categoryId,
        QLandmarkManager::Error *error, QString *errorString)
    {
    Q_ASSERT(error);
    ...
    *errorString = "";
    ...
    if (removed)
        ...
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::categoryRemoved,
                QList<QLandmarkId> (), categoryIds);
        }
...
/*!
 ...
 \a errorString.
 */
bool LandmarkManagerEngineSymbianPrivate::importLandmarks(QIODevice *device,
    ...
    *errorString = "";
    ...
    if (err == KErrNone)
        ...
        status = true;
        m_LmEventObserver.handleLandmarkEvent(
                LandmarkEventObserver::landmarkAdded, importedIds);
        }
    else
        handleSymbianError(err, error, errorString);
...
/*!
 ...
 \a errorString.
 */
bool LandmarkManagerEngineSymbianPrivate::exportLandmarks(QIODevice *device,
        const QString &format, QList<QLandmarkId> landmarkIds,
    ...
    *errorString = "";
    ...
    QList<QLandmarkSortOrder> order;
    ...
    if (landmarkIds.isEmpty())
        fetchedLandmarkId = this->landmarkIds(QLandmarkFilter(),
                KAllLandmarks, KDefaultIndex, order, error, errorString);
    else
        fetchedLandmarkId = this->landmarkIds(filter, KAllLandmarks,
                KDefaultIndex, order, error, errorString);
...
QLandmarkManager::SupportLevel LandmarkManagerEngineSymbianPrivate::filterSupportLevel(
        const QLandmarkFilter &filter, QLandmarkManager::Error *error,
        QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
    ...
    *errorString = "";
    ...
    switch (filter.type())
        ...
        case QLandmarkFilter::NameFilter:
            ...
            if ((nameFilter.matchFlags() == QLandmarkFilter::MatchContains)
        ...
        case QLandmarkFilter::AttributeFilter:
            ...
            QLandmarkAttributeFilter::OperationType opType =
                    attributeFilter.operationType();

            if (opType == QLandmarkAttributeFilter::OrOperation
                    && (QSysInfo::s60Version() == QSysInfo::SV_S60_3_1
                            || QSysInfo::s60Version() == QSysInfo::SV_S60_3_2
                            || QSysInfo::s60Version() == QSysInfo::SV_S60_5_0))
                {
            ...
            if (keyList.size() > 0)
                ...
                for (int i = 0; i < keyList.size(); ++i)
                    {
                    if (attributeFilter.matchFlags(keyList.at(i))
                            & QLandmarkFilter::MatchCaseSensitive)
                        {
                        break;
                        }
                    ...
                    if ((opType == QLandmarkAttributeFilter::AndOperation)
                            && attributeFilter.matchFlags(keyList.at(i))
                                    & QLandmarkFilter::MatchContains)
                        {
                        break;
                        }
                    }
                ...
                if (keyList.contains("longitude") || keyList.contains(
                        "latitude") || keyList.contains("radius")
                        || keyList.contains("altitude") || keyList.contains(
                        "radius") || keyList.contains("iconurl"))
                    {
...
QLandmarkManager::SupportLevel LandmarkManagerEngineSymbianPrivate::sortOrderSupportLevel(
    ...
    *errorString = "";
    ...
    QLandmarkManager::SupportLevel supportLevel =
            QLandmarkManager::NativeSupport;
...
bool LandmarkManagerEngineSymbianPrivate::isFeatureSupported(
        QLandmarkManager::LandmarkFeature feature,
        QLandmarkManager::Error *error, QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::isReadOnly(
        QLandmarkManager::Error *error, QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
...
bool LandmarkManagerEngineSymbianPrivate::isReadOnly(
    ...
    *errorString = "";
    ...
    if (LandmarkUtility::validLocalId(categoryId.localId()))
        {
        result = LandmarkUtility::isGlobalCategoryId(m_LandmarkCatMgr,
                categoryId);
        }
    return result;
    }
...
QStringList LandmarkManagerEngineSymbianPrivate::supportedFormats(
        QLandmarkManager::TransferOperation operation,
        QLandmarkManager::Error *error, QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
 ...
 */
void LandmarkManagerEngineSymbianPrivate::requestDestroyed(
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::startRequest(
        QLandmarkAbstractRequest* request)
    {
    bool result = false;
    TRAPD(errorId,
    ...
    if (errorId != KErrNone)
        ...
        switch (request->type())
            ...
            case QLandmarkAbstractRequest::LandmarkIdFetchRequest:
                ...
                QLandmarkManagerEngineSymbian::updateLandmarkIdFetchRequest(
                        lmIdFetchRequest, QList<QLandmarkId> (), error,
            ...
            case QLandmarkAbstractRequest::CategoryIdFetchRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkCategoryIdFetchRequest(
                        catIdFetchRequest, QList<QLandmarkCategoryId> (),
                        error, errorString,
            ...
            case QLandmarkAbstractRequest::LandmarkFetchRequest:
                ...
                QLandmarkFetchRequest *lmfetchRequest =
                        static_cast<QLandmarkFetchRequest*> (request);

                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkFetchRequest(
                        lmfetchRequest, QList<QLandmark> (), error,
                        errorString, QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::CategoryFetchRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkCategoryFetchRequest(
                        catFetchRequest, QList<QLandmarkCategory> (), error,
                        errorString, QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::LandmarkFetchByIdRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkFetchByIdRequest(
                        lmFetchRequest, QList<QLandmark> (), error,
            ...
            case QLandmarkAbstractRequest::CategoryFetchByIdRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkCategoryFetchByIdRequest(
                        catFetchRequest, QList<QLandmarkCategory> (), error,
                        errorString, QMap<int, QLandmarkManager::Error> (),
                        QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::LandmarkSaveRequest:
                ...
                QLandmarkSaveRequest *lmSaveRequest =
                        static_cast<QLandmarkSaveRequest*> (request);

                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkSaveRequest(
                        lmSaveRequest, QList<QLandmark> (), error,
                        errorString, QMap<int, QLandmarkManager::Error> (),
                        QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::LandmarkRemoveRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkRemoveRequest(
                        lmRemoveRequest, error, errorString, QMap<int,
                                QLandmarkManager::Error> (),
                        QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::CategorySaveRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkCategorySaveRequest(
                        catSaveRequest, QList<QLandmarkCategory> (), error,
                        errorString, QMap<int, QLandmarkManager::Error> (),
                        QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::CategoryRemoveRequest:
                ...
                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkCategoryRemoveRequest(
                        catRemoveRequest, error, errorString, QMap<int,
                                QLandmarkManager::Error> (),
                        QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::ImportRequest:
                ...
                QLandmarkImportRequest *importRequest =
                        static_cast<QLandmarkImportRequest*> (request);

                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkImportRequest(
                        importRequest, QList<QLandmarkId> (), error,
                        errorString, QLandmarkAbstractRequest::FinishedState);
            ...
            case QLandmarkAbstractRequest::ExportRequest:
                ...
                QLandmarkExportRequest *exportRequest =
                        static_cast<QLandmarkExportRequest*> (request);

                // for resultsAvailable signal
                QLandmarkManagerEngineSymbian::updateLandmarkExportRequest(
                        exportRequest, error, errorString,
                        QLandmarkAbstractRequest::FinishedState);
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::startRequestL(
        QLandmarkAbstractRequest* request)
    {
    bool result = false;
    ...
    switch (request->type())
        ...
        case QLandmarkAbstractRequest::CategoryFetchRequest:
            ...
            if (request->type()
                    == QLandmarkAbstractRequest::CategoryFetchRequest)
                {
                ...
                if (catFetchRequest->sorting().direction()
                        == Qt::AscendingOrder)
                    {
                    sortPref
                            = CPosLmCategoryManager::ECategorySortOrderNameAscending;
                    }
                else if (catFetchRequest->sorting().direction()
                        == Qt::DescendingOrder)
                    {
                    sortPref
                    ...
                    == QLandmarkAbstractRequest::CategoryIdFetchRequest)
                {
                ...
                if (catIdFetchRequest->sorting().direction()
                        == Qt::AscendingOrder)
                    {
                    sortPref
                            = CPosLmCategoryManager::ECategorySortOrderNameAscending;
                    }
                else if (catIdFetchRequest->sorting().direction()
                        == Qt::DescendingOrder)
                    {
                    sortPref
                            = CPosLmCategoryManager::ECategorySortOrderNameDescending;
            ...
            CPosLmCatNameCriteria* catNameCriteria =
                    CPosLmCatNameCriteria::NewLC();
            catNameCriteria->SetSearchPatternL(KDefaultTextCriteria);
            CPosLandmarkSearch* landmarkSearch = CPosLandmarkSearch::NewL(
                    *m_LandmarkDb);
            CleanupStack::PushL(landmarkSearch);
            CPosLmOperation* lmOperation =
                    landmarkSearch->StartCategorySearchL(*catNameCriteria,
                            sortPref);
        ...
        case QLandmarkAbstractRequest::LandmarkFetchRequest:
            ...
            if (request->type()
                    == QLandmarkAbstractRequest::LandmarkIdFetchRequest)
                {
                QLandmarkIdFetchRequest *lmIdFetchRequest =
                        static_cast<QLandmarkIdFetchRequest *> (request);
                ...
                if (lmIdFetchRequest->limit() <= 0)
                    ...
                    maxMatches = lmIdFetchRequest->limit()
                            + lmIdFetchRequest->offset();
                }

            if (request->type()
                    == QLandmarkAbstractRequest::LandmarkFetchRequest)
                {
                ...
                if (lmFetchRequest->limit() <= 0)
                    ...
                    maxMatches = lmFetchRequest->limit()
                            + lmFetchRequest->offset();
                }

            bool multiplefetch = false;
            if (filter.type() == QLandmarkFilter::IntersectionFilter
                    || filter.type() == QLandmarkFilter::UnionFilter)
                {
                multiplefetch = true;
                }
            ...
            if (filter.type() == QLandmarkFilter::LandmarkIdFilter
                ...
                if (!sortOrders.isEmpty() && !multiplefetch)
                    ...
                    for (int i = 0; i < sortOrders.size(); ++i)
                        {
                        if (sortOrders.at(i).type()
                                != QLandmarkSortOrder::NameSort)
                            {
                            sortRequired = true;
                            break;
                            }
                ...
                if (sortRequired || multiplefetch)
                    ...
                    // create request AO and start async request
                    CLandmarkRequestAO* requestAO = CLandmarkRequestAO::NewL(
                            this);
                    CleanupStack::PushL(requestAO);
                    TInt retn = m_RequestHandler.AddAsyncRequest(request,
                            requestAO);
                    if (retn != KErrNone)
                        {
                        CleanupStack::Pop(requestAO);
            ...
            if (sortOrders.isEmpty())
                lmOperation = getOperationL(landmarkSearch, searchCriteria,
                        QLandmarkSortOrder(), maxMatches);
            else
                lmOperation = getOperationL(landmarkSearch, searchCriteria,
                        sortOrders.at(0), maxMatches);
            if (lmOperation == NULL)
                {
        ...
        case QLandmarkAbstractRequest::ImportRequest:
            ...
            TPosLmItemId catId =
                    LandmarkUtility::convertToSymbianLandmarkCategoryId(
                            importRequest->categoryId());

            // check for AttachSingleCategory transfer option & association of provided category id.
            if ((catId == KPosLmNullItemId || (managerUri()
                    != importRequest->categoryId().managerUri()))
                    && (importRequest->transferOption()
                            == QLandmarkManager::AttachSingleCategory))
                User::Leave(KErrArgument);

            // check availability of category from provided category id.
            if (importRequest->transferOption()
                    == QLandmarkManager::AttachSingleCategory)
                {
                CPosLandmarkCategory *tempCategory =
                        m_LandmarkCatMgr->ReadCategoryLC(catId);
            ...
            if (outputdevice)
                ...
                QString filename = LandmarkUtility::preparePath(
                        filePath->fileName());
                ...
                RFile fileName;
                TInt err(fileName.Open(fs, pathPtr, EFileShareReadersOnly
                        | EFileRead));
                ...
                if (err == KErrNone && (dataType.iConfidence
                        == CApaDataRecognizerType::EProbable
                        || dataType.iConfidence
                                == CApaDataRecognizerType::ECertain
                        || dataType.iConfidence
                                == CApaDataRecognizerType::EPossible))
                    {

                    parser = CPosLandmarkParser::NewL(
                            dataType.iDataType.Des8());
                ...
                importOperation = m_LandmarkDb->ImportLandmarksL(*parser,
                        transferOption);
                ...
                outputdevice
                        = dynamic_cast<QBuffer *> (importRequest->device());
                if (outputdevice)
                    {
                    ...
                    HBufC8* packageFormat =
                            LandmarkUtility::landmarkPackageFormatsStrL(
        ...
        case QLandmarkAbstractRequest::ExportRequest:
            ...
            QString errorString = "";
            QStringList exportFormats = supportedFormats(
                    QLandmarkManager::ExportOperation, &error, &errorString);
            ...
            if (!exportFormats.contains(exportRequest->format(),
                    Qt::CaseInsensitive))
                {
                qDebug() << "Not Supported Export Format Type = "
            ...
            if (!exportRequest->landmarkIds().size())
                ...
                foreach(const QLandmarkId& id,exportRequest->landmarkIds())
                        selectedLandmarks.AppendL(
                                LandmarkUtility::convertToSymbianLandmarkId(
                                        id));
                }

            exportedLandmarkIds = LandmarkUtility::convertToQtLandmarkIds(
            ...
            CPosLandmarkEncoder* encoder = CPosLandmarkEncoder::NewL(
                    KPosMimeTypeLandmarkCollectionXml);
            ...
            if (outputdevice)
                ...
                QString filePathName = LandmarkUtility::preparePath(
                        filePath->fileName());
                ...
                exportOperation = m_LandmarkDb->ExportLandmarksL(*encoder,
                        selectedLandmarks, transferOption);
                ...
                outputdevice
                        = dynamic_cast<QBuffer *> (exportRequest->device());
                if (outputdevice)
                    {
                    ...
                    // Performs export of landmarks  and cleanup of CPosLmOperation
                    exportOperation = m_LandmarkDb->ExportLandmarksL(
                            *encoder, selectedLandmarks, transferOption);
                    }
                else
                    {
                    // Invalid device has been provided.
            ...
            CLandmarkRequestAO* requestAO = CLandmarkRequestAO::NewL(this,
                    exportOperation);
            CleanupStack::PushL(requestAO);
            TInt retn = m_RequestHandler.AddAsyncRequest(exportRequest,
                    requestAO);
            if (retn != KErrNone)
                {
                User::Leave(retn);
            ...
            // start the request & transfer landmarkSearch object ownership
            requestAO->StartRequest(NULL);
            requestAO->SetExportData(encoder, fs, exportPath, bufferPath,
                    exportedLandmarkIds);
            CleanupStack::Pop(requestAO);
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::cancelRequest(
        QLandmarkAbstractRequest* request)
    {
...
/*!
 ...
 \sa startRequest()
 */
bool LandmarkManagerEngineSymbianPrivate::waitForRequestFinished(
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::saveLandmarkInternalL(
        QLandmark* landmark, QLandmarkManager::Error *error,
    ...
    *changed = false;
    ...
    if (!landmark->landmarkId().managerUri().isEmpty()
            && landmark->landmarkId().managerUri() != managerUri())
        {
        *error = QLandmarkManager::DoesNotExistError;
    ...
    bool categoryExists = LandmarkUtility::validCategoriesExist(
            m_LandmarkCatMgr, landmark, managerUri());
    if (!categoryExists)
        {
        *error = QLandmarkManager::BadArgumentError;
    ...
    if (landmarkId.localId().isEmpty())
        {
        symbianLandmark
                = LandmarkUtility::convertToSymbianLandmarkL(landmark);
        TPosLmItemId savedsymbianLmId = KPosLmNullItemId;
        savedsymbianLmId = m_LandmarkDb->AddLandmarkL(*symbianLandmark);
        QLandmarkId savedQtLmId = LandmarkUtility::convertToQtLandmarkId(
                managerUri(), savedsymbianLmId);
        ...
        *errorString
        ...
        TPosLmItemId symbianLmId = LandmarkUtility::getSymbianLandmarkId(
                landmark);
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::removeLandmarkInternalL(
        const QLandmarkId &landmarkId, QLandmarkManager::Error *error,
    ...
    *removed = false;
    ...
    TPosLmItemId symbianLmId = LandmarkUtility::convertToSymbianLandmarkId(
            landmarkId);
 ...
 * 
 */
QLandmark LandmarkManagerEngineSymbianPrivate::fetchLandmarkL(
        const QLandmarkId &landmarkId, QLandmarkManager::Error *error,
        QString *errorString) const
    {
    Q_ASSERT(error);
    ...
    if (landmarkId.managerUri() != managerUri())
        ...
        *error = QLandmarkManager::DoesNotExistError;
        *errorString
                = "Landmark not found as landmark id comes from different landmark manager.";
        return QLandmark();
        }
    else if (!LandmarkUtility::validLocalId(landmarkId.localId()))
        {
    ...
    if (symbianLandmark)
        {
        QLandmark* qtLandmark = LandmarkUtility::convertToQtLandmark(
                managerUri(), symbianLandmark);
        CleanupStack::PopAndDestroy(symbianLandmark);
        return *qtLandmark;
        }
    else
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::saveCategoryInternalL(
        QLandmarkCategory* category, QLandmarkManager::Error *error,
        QString *errorString, bool *added, bool *changed)
    {
    Q_ASSERT(error);
    ...
    *changed = false;
    ...
    if (!category->categoryId().managerUri().isEmpty()
            && category->categoryId().managerUri() != managerUri())
        {
        *error = QLandmarkManager::DoesNotExistError;
        *errorString
                = "Landmark Category id comes from different landmark manager.";
        return result;
        }
    ...
    if (categoryId.localId().isEmpty())
        {
        symbiancat = LandmarkUtility::convertToSymbianLandmarkCategoryL(
                category);
        ...
        *errorString
        ...
        if (LandmarkUtility::isGlobalCategoryId(m_LandmarkCatMgr,
                category->categoryId()))
            {
        ...
        if (symbiancat)
            {
            LandmarkUtility::setSymbianLandmarkCategoryL(*symbiancat,
                    category);
...
bool LandmarkManagerEngineSymbianPrivate::removeCategoryInternalL(
    ...
    CPosLandmarkCategory* cat = m_LandmarkCatMgr->ReadCategoryLC(
            symbianCategoryId);
    if (cat)
...
QLandmarkCategory LandmarkManagerEngineSymbianPrivate::fetchCategoryL(
        const QLandmarkCategoryId &landmarkCategoryId,
        QLandmarkManager::Error *error, QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
    ...
    if (&landmarkCategoryId == 0)
        ...
        *error = QLandmarkManager::DoesNotExistError;
        *errorString
                = "Category not found because the category id is invalid or empty";
        return QLandmarkCategory();
        }
    ...
    TPosLmItemId symbianCategoryId =
            LandmarkUtility::convertToSymbianLandmarkCategoryId(
                    landmarkCategoryId);
    CPosLandmarkCategory* symbiancat = symbiancat
            = m_LandmarkCatMgr->ReadCategoryLC(symbianCategoryId);
    if (symbiancat)
        {
        QLandmarkCategory* qtCat =
                LandmarkUtility::convertToQtLandmarkCategory(managerUri(),
                        symbiancat);
        CleanupStack::PopAndDestroy(symbiancat);
        return *qtCat;
        }
...
QList<QLandmarkCategoryId> LandmarkManagerEngineSymbianPrivate::fetchCategoryIdsL(
        const QLandmarkNameSort &nameSort, QLandmarkManager::Error *error,
        QString *errorString) const
    {
    Q_ASSERT(error);
    Q_ASSERT(errorString);
    ...
    *errorString = "";
    ...
    CPosLmItemIterator* iterator = m_LandmarkCatMgr->CategoryIteratorL(
            sortPref);
    ...
    QList<QLandmarkCategoryId> qtCatIds =
            LandmarkUtility::convertToQtLandmarkCategoryIds(managerUri(),
                    idArray);
    idArray.Close();
    return qtCatIds;
    }
    ...
    switch (filter.type())
        ...
        case QLandmarkFilter::NameFilter:
            ...
            if (nameFilter.matchFlags() & QLandmarkFilter::MatchCaseSensitive)
                {
                qDebug()
                        << "Symbian Landmarks Apis don't support MatchCaseSensitivity.";
                User::Leave(KErrNotSupported);
                }
            ...
            HBufC* filterbuf = HBufC::NewL(nameFilter.name().size()
                    + KExtrachars);
            TPtr filterName = filterbuf->Des();

            HBufC* filterbufcont = HBufC::NewL(nameFilter.name().size()
                    + KExtrachars);
        ...
        case QLandmarkFilter::LandmarkIdFilter:
            ...
            RArray<TPosLmItemId> symLmIds =
                    LandmarkUtility::getSymbianLandmarkIds(qtLmIds);
            if (symLmIds.Count() <= 0)
                {
                User::Leave(KErrArgument);
                }
            ...
            CPosLmIdListCriteria *idListCriteria =
        ...
        case QLandmarkFilter::CategoryFilter:
            ...
            CPosLmCategoryCriteria* categorySearchCriteria =
        ...
        case QLandmarkFilter::ProximityFilter:
            ...
            if (LandmarkUtility::isValidLat(qCoord.latitude())
                    && LandmarkUtility::isValidLong(qCoord.longitude()))
                {
                symbianCoord.SetCoordinate(qCoord.latitude(),
                        qCoord.longitude());
                }
            else
                {
            ...
            CPosLmNearestCriteria* nearestCriteria =
                    CPosLmNearestCriteria::NewLC(symbianCoord);
        ...
        case QLandmarkFilter::BoxFilter:
            ...
            const TReal64& eastlong =
                    boxFilter.boundingBox().bottomRight().longitude();
            const TReal64& southlat =
                    boxFilter.boundingBox().bottomRight().latitude();
            const TReal64& northlat =
                    boxFilter.boundingBox().topLeft().latitude();
            const TReal64& westlong =
                    boxFilter.boundingBox().topLeft().longitude();

            if (southlat > northlat)
                User::Leave(KErrArgument);
            if (LandmarkUtility::isValidLong(eastlong)
                    && LandmarkUtility::isValidLat(southlat)
                    && LandmarkUtility::isValidLat(northlat)
                    && LandmarkUtility::isValidLong(westlong))
                {
        ...
        case QLandmarkFilter::AttributeFilter:
            ...
            for (int i = 0; i < keyList.size(); ++i)
                ...
                if (attributeFilter.matchFlags(keyList.at(i))
                    ...
                    qDebug()
                            << "Symbian Landmarks Apis don't support MatchCaseSensitive based search.";
                    User::Leave(KErrNotSupported);
                    }
                }

            // the attributes search which cannot be used in CPosLmTextCriteria
            if (keyList.contains("longitude") || keyList.contains("latitude")
                    || keyList.contains("radius") || keyList.contains(
                    "altitude") || keyList.contains("radius")
            ...
            for (int i = 0; i < keyList.size(); ++i)
                ...
                if ((keyList.at(i) == "latitude") || (keyList.at(i)
                        == "longitude") || (keyList.at(i) == "altitude")
                        || (keyList.at(i) == "radius") || (keyList.at(i)
                        == "iconurl"))
                    continue;
                ...
                HBufC* filterbuf = HBufC::NewL(keyValue.toString().size()
                        + KExtrachars);
                TPtr filterName = filterbuf->Des();

                HBufC* filterbufcont = HBufC::NewL(keyValue.toString().size()
                        + KExtrachars);
                TPtr filterNamecont = filterbufcont->Des();
                ...
                if (keyValue.isValid())
                    ...
                    QLandmarkFilter::MatchFlags flag =
                            attributeFilter.matchFlags(keyList.at(i));
                    ...
                    switch (flag)
                        ...
                        case QLandmarkFilter::MatchEndsWith:
                            {
                            filterName.Copy(KDefaultTextCriteria);
                            filterName.Append(keyValue.toString().utf16(),
                                    keyValue.toString().size());
                            break;
                            }
                        ...
                        case QLandmarkFilter::MatchContains:
                            ...
                            filterNamecont.Append(
                                    keyValue.toString().utf16(),
                                    keyValue.toString().size());
                            filterNamecont.Append(KDefaultTextCriteria);
                            break;
                            }
                ...
                QString filterStr1((QChar*) (filterName.Ptr()),
                        filterName.Length());
                qDebug() << "filter string1  = " << filterStr1;

                if (filterNamecont.Length() > 0)
                    {
                    QString filterStr2((QChar*) (filterNamecont.Ptr()),
                            filterNamecont.Length());
                    qDebug() << "filter string2  = " << filterStr2;
                    }
                ...
                if (keyList.at(i) == "name")
                    ...
                    CPosLmTextCriteria* textPhoneCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    ...
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsPhone =
                                CPosLmTextCriteria::NewLC();
                        textContainsPhone->SetTextL(filterNamecont);
                        textContainsPhone->SetPositionFieldsToSearchL(
                                afieldArray);
                    ...
                    CPosLmTextCriteria* textCountryCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    textCountryCriteria->SetTextL(filterName);
                    textCountryCriteria->SetPositionFieldsToSearchL(
                            afieldArray);
                    compositeCriteria->AddArgument(textCountryCriteria);
                    CleanupStack::Pop(textCountryCriteria);
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsCountry =
                                CPosLmTextCriteria::NewLC();
                        textContainsCountry->SetTextL(filterNamecont);
                        textContainsCountry->SetPositionFieldsToSearchL(
                                afieldArray);
                    ...
                    CPosLmTextCriteria* textStateCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    ...
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsState =
                                CPosLmTextCriteria::NewLC();
                        textContainsState->SetTextL(filterNamecont);
                        textContainsState->SetPositionFieldsToSearchL(
                                afieldArray);
                    ...
                    {
                    CPosLmTextCriteria* textCountyCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    textCountyCriteria->SetTextL(filterName);
                    textCountyCriteria->SetPositionFieldsToSearchL(
                            afieldArray);
                    compositeCriteria->AddArgument(textCountyCriteria);
                    CleanupStack::Pop(textCountyCriteria);
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsCounty =
                                CPosLmTextCriteria::NewLC();
                        textContainsCounty->SetTextL(filterNamecont);
                        textContainsCounty->SetPositionFieldsToSearchL(
                                afieldArray);
                    ...
                    {
                    CPosLmTextCriteria* textCityCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    ...
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsCity =
                                CPosLmTextCriteria::NewLC();
                        textContainsCity->SetTextL(filterNamecont);
                        textContainsCity->SetPositionFieldsToSearchL(
                                afieldArray);
                    ...
                    {
                    CPosLmTextCriteria* textStreetCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    textStreetCriteria->SetTextL(filterName);
                    textStreetCriteria->SetPositionFieldsToSearchL(
                            afieldArray);
                    compositeCriteria->AddArgument(textStreetCriteria);
                    CleanupStack::Pop(textStreetCriteria);
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsStreet =
                                CPosLmTextCriteria::NewLC();
                        textContainsStreet->SetTextL(filterNamecont);
                        textContainsStreet->SetPositionFieldsToSearchL(
                                afieldArray);
                    ...
                    {
                    CPosLmTextCriteria* textPostCodeCriteria =
                            CPosLmTextCriteria::NewLC();
                    afieldArray.InsertInOrder(
                            LandmarkUtility::positionFieldId(keyList.at(i)));
                    textPostCodeCriteria->SetTextL(filterName);
                    textPostCodeCriteria->SetPositionFieldsToSearchL(
                            afieldArray);
                    compositeCriteria->AddArgument(textPostCodeCriteria);
                    CleanupStack::Pop(textPostCodeCriteria);
                    if (filterNamecont.Length() > 0)
                        {
                        CPosLmTextCriteria* textContainsPostCode =
                                CPosLmTextCriteria::NewLC();
                        textContainsPostCode->SetTextL(filterNamecont);
                        textContainsPostCode->SetPositionFieldsToSearchL(
                                afieldArray);
...
CPosLmOperation* LandmarkManagerEngineSymbianPrivate::getOperationL(
        CPosLandmarkSearch* landmarkSearch,
        CPosLmSearchCriteria* searchCriteria, QLandmarkSortOrder qtSortOrder,
    ...
    CPosLmPartialReadParameters* readParams =
 ...
 * 
 */
QList<QLandmarkId> LandmarkManagerEngineSymbianPrivate::searchWithFilterL(
        QLandmarkFilter filter, QLandmarkSortOrder qtSortOrder,
        int maxMatches) const
    {
    QList<QLandmarkId> result;
    // use native serarch
    ...
    CPosLandmarkSearch* landmarkSearch = CPosLandmarkSearch::NewL(
            *m_LandmarkDb);
    CleanupStack::PushL(landmarkSearch);
    ExecuteAndDeleteLD(getOperationL(landmarkSearch, searchCriteria,
            qtSortOrder, maxMatches));
    TPosLmItemId lmId;
    CPosLmItemIterator *iterator = landmarkSearch->MatchIteratorL();
    while ((lmId = iterator->NextL()) != KPosLmNullItemId)
        {
        result.append(LandmarkUtility::convertToQtLandmarkId(managerUri(),
                lmId));
        }
...
void LandmarkManagerEngineSymbianPrivate::handleSymbianError(TInt errorId,
    ...
    switch (errorId)
        ...
        case KErrPosLmUnknownFormat:
            ...
            *error = QLandmarkManager::NotSupportedError;
            *errorString
                    = "Unknown Landmark Package format file or data to import-export.";
            break;
            }
        ...
        case KErrPermissionDenied:
            ...
            *error = QLandmarkManager::PermissionsError;
            *errorString
                    = "Permission Denied, Invalid capabilities assigned.";
            break;
            }
        ...
        case KErrNotFound:
            ...
            *error = QLandmarkManager::DoesNotExistError;
            *errorString
                    = "None of the existing landmarks/categories match the given id.";
            break;
            }
        ...
        case KErrAlreadyExists:
            ...
            *errorString
                    = "Category Name already exists, please provide unique category name.";
            break;
            }
        case KErrPosLmNotInitialized:
            ...
            *errorString
                    = "Mangager is not constructed and initialized properly.";
            break;
            }
        default:
            {
            *error = QLandmarkManager::UnknownError;
            *errorString = QString("Symbian Landmarks Error = %1").arg(
                    errorId);
            qDebug() << *errorString;
            break;
            }
 ...
 * HandleCompletion
 */
void LandmarkManagerEngineSymbianPrivate::HandleCompletionL(
        CLandmarkRequestData* aData)
    {
    ...
    switch (aData->iQtRequest->type())
        ...
        case QLandmarkAbstractRequest::LandmarkIdFetchRequest:
            ...
            if (error != QLandmarkManager::NoError)
                ...
                CPosLmItemIterator *iterator =
                        aData->iLandmarkSearch->MatchIteratorL();
                ...
                if (lmcount > 0 && lmIdFetchRequest->offset() < lmcount)
                    {
                    iterator->GetItemIdsL(symbianLmIds, 0,
                            iterator->NumOfItemsL());

                    aData->iLandmarkIds
                            = LandmarkUtility::convertToQtLandmarkIds(
                                    managerUri(), symbianLmIds);
                    symbianLmIds.Close();

                    sortFetchedLmIds(lmIdFetchRequest->limit(),
                            lmIdFetchRequest->offset(),
                            lmIdFetchRequest->sorting(), aData->iLandmarkIds,
                            lmIdFetchRequest->filter().type(), &error,
                            &errorString);
                    }
                }
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkIdFetchRequest(
                    lmIdFetchRequest, aData->iLandmarkIds, error,
                    errorString, QLandmarkAbstractRequest::FinishedState);
        ...
        case QLandmarkAbstractRequest::CategoryIdFetchRequest:
            ...
            if (error != QLandmarkManager::NoError)
                ...
                if (iterator->NumOfItemsL() > 0)
                    {
                    iterator->GetItemIdsL(symbianCatIds, 0,
                            iterator->NumOfItemsL());
                    }
                aData->iCategoryIds
                        = LandmarkUtility::convertToQtLandmarkCategoryIds(
                                managerUri(), symbianCatIds);
                symbianCatIds.Close();
                }
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkCategoryIdFetchRequest(
                    catIdFetchRequest, aData->iCategoryIds, error,
                    errorString, QLandmarkAbstractRequest::FinishedState);
        ...
        case QLandmarkAbstractRequest::LandmarkFetchRequest:
            ...
            if (error != QLandmarkManager::NoError)
                ...
                if (lmcount > 0 && lmfetchRequest->offset() < lmcount)
                    ...
                    if (sortFetchedLmIds(lmfetchRequest->limit(),
                            lmfetchRequest->offset(),
                            lmfetchRequest->sorting(), aData->iLandmarkIds,
                            lmfetchRequest->filter().type(), &error,
                            &errorString))
                        {
                        ...
                        foreach (const QLandmarkId& lmId,aData->iLandmarkIds)
                                ...
                                qtLandmark = landmark(lmId, &error,
                                        &errorString);
                                if (error != QLandmarkManager::NoError)
                                    {
                                    aData->iLandmarks.clear();
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkFetchRequest(
                    lmfetchRequest, aData->iLandmarks, error, errorString,
                    QLandmarkAbstractRequest::FinishedState);
        ...
        case QLandmarkAbstractRequest::CategoryFetchRequest:
            ...
            if (error != QLandmarkManager::NoError)
                ...
                foreach (const QLandmarkCategoryId& lmCatId,aData->iCategoryIds)
                        ...
                        // use landmark fetch method to get landmark from landmark id
                        qtLmCategory
                                = category(lmCatId, &error, &errorString);
                        if (error != QLandmarkManager::NoError)
                            {
                            aData->iCategories.clear();
                            break;
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkCategoryFetchRequest(
                    catFetchRequest, aData->iCategories, error, errorString,
                    QLandmarkAbstractRequest::FinishedState);
        ...
        case QLandmarkAbstractRequest::LandmarkFetchByIdRequest:
            ...
            QLandmarkManagerEngineSymbian::updateLandmarkFetchByIdRequest(
                    lmFetchRequest, aData->iLandmarks, error, errorString,
                    aData->iErrorMap, QLandmarkAbstractRequest::FinishedState);
        ...
        case QLandmarkAbstractRequest::LandmarkSaveRequest:
            ...
            if (error == QLandmarkManager::CancelError)
                {
                for (int i = aData->iOpCount; i
                        < lmSaveRequest->landmarks().size(); ++i)
                    {
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkSaveRequest(
                    lmSaveRequest, aData->iLandmarks, error, errorString,
                    aData->iErrorMap, QLandmarkAbstractRequest::FinishedState);
            ...
            if (aData->iAddedLandmarkIds.size() > 0)
                {
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::landmarkAdded,
            ...
            if (aData->iChangedLandmarkIds.size() > 0)
                {
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::landmarkUpdated,
        ...
        case QLandmarkAbstractRequest::LandmarkRemoveRequest:
            ...
            if (error == QLandmarkManager::CancelError)
                {
                for (int i = aData->iOpCount; i
                        < lmRemoveRequest->landmarkIds().size(); ++i)
                    {
                    aData->iErrorMap.insert(i, QLandmarkManager::CancelError);
                    }
                }
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkRemoveRequest(
                    lmRemoveRequest, error, errorString, aData->iErrorMap,
                    QLandmarkAbstractRequest::FinishedState);
            ...
            if (aData->iRemovedLandmarkIds.size() > 0)
                {
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::landmarkRemoved,
        ...
        case QLandmarkAbstractRequest::CategorySaveRequest:
            ...
            if (error == QLandmarkManager::CancelError)
                {
                for (int i = aData->iOpCount; i
                        < catSaveRequest->categories().size(); ++i)
                    {
                    aData->iErrorMap.insert(i, QLandmarkManager::CancelError);
                    }
                }
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkCategorySaveRequest(
                    catSaveRequest, aData->iCategories, error, errorString,
                    aData->iErrorMap, QLandmarkAbstractRequest::FinishedState);
            ...
            if (aData->iAddedCategoryIds.size() > 0)
                {
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::categoryAdded, QList<
            ...
            if (aData->iChangedCategoryIds.size() > 0)
                {
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::categoryUpdated, QList<
        ...
        case QLandmarkAbstractRequest::CategoryRemoveRequest:
            ...
            if (error == QLandmarkManager::CancelError)
                {
                for (int i = aData->iOpCount; i
                        < catRemoveRequest->categoryIds().size(); ++i)
                    {
                    aData->iErrorMap.insert(i, QLandmarkManager::CancelError);
                    }
                }
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkCategoryRemoveRequest(
                    catRemoveRequest, error, errorString, aData->iErrorMap,
                    QLandmarkAbstractRequest::FinishedState);
            ...
            if (aData->iRemovedCategoryIds.size() > 0)
                {
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::categoryRemoved, QList<
        ...
        case QLandmarkAbstractRequest::ImportRequest:
            ...
            if (aData->iOwnerAO->GetOperation())
                ...
                CPosLmItemIterator* iter =
                        m_LandmarkDb->ImportedLandmarksIteratorL(
                ...
                if (importRequest->transferOption()
                    ...
                    ExecuteAndDeleteLD(
                            m_LandmarkCatMgr->AddCategoryToLandmarksL(catId,
                                    landmarkIds));
            ...
            // for resultsAvailable signal
            QLandmarkManagerEngineSymbian::updateLandmarkImportRequest(
                    importRequest, importedLandmarkIds, error, errorString,
                    QLandmarkAbstractRequest::FinishedState);
        ...
        case QLandmarkAbstractRequest::ExportRequest:
            ...
            if (aData->iErrorId == KErrNone && aData->iLandmarkEncoder)
                ...
                ExecuteAndDeleteLD(
                        aData->iLandmarkEncoder->FinalizeEncodingL());

                // Write to the QBuffer
                if (aData->iExportBuffer)
                    {
                    int n = aData->iExportBuffer->Size();
                    HBufC8 *buffer = HBufC8::New(aData->iExportBuffer->Size()
                            + 1);
 ...
 */
void LandmarkManagerEngineSymbianPrivate::HandleExecutionL(
        CLandmarkRequestData* aData, TRequestStatus& aRequest)
    {
    QLandmarkManager::Error error = QLandmarkManager::NoError;
    QString errorString = 0;
    ...
    switch (aData->iQtRequest->type())
        ...
        case QLandmarkAbstractRequest::LandmarkSaveRequest:
            ...
            if (aData->iOpCount < lmSaveRequest->landmarks().size())
                ...
                QLandmark qtLm = (lmSaveRequest->landmarks()).at(
                        aData->iOpCount);
                TRAPD(err,
                ...
                if (!saveResult)
                    ...
                    aData->iLandmarks.append((lmSaveRequest->landmarks()).at(
                            aData->iOpCount));
                    }
        ...
        case QLandmarkAbstractRequest::LandmarkRemoveRequest:
            ...
            if (aData->iOpCount < lmRemoveRequest->landmarkIds().size())
                ...
                QLandmarkId qtLmId = (lmRemoveRequest->landmarkIds()).at(
                        aData->iOpCount);
                TRAPD(err,
        ...
        case QLandmarkAbstractRequest::CategorySaveRequest:
            ...
            if (aData->iOpCount < saveCategoryRequest->categories().count())
                ...
                QLandmarkCategory qtCat =
                        (saveCategoryRequest->categories()).at(
                                aData->iOpCount);
                TRAPD(err,
        ...
        case QLandmarkAbstractRequest::CategoryRemoveRequest:
            ...
            if (aData->iOpCount < catRemoveRequest->categoryIds().size())
                ...
                QLandmarkCategoryId qtCatId =
                        (catRemoveRequest->categoryIds()).at(aData->iOpCount);
                TRAPD(err,
        ...
        case QLandmarkAbstractRequest::LandmarkFetchByIdRequest:
            ...
            if (aData->iOpCount < fetchRequest->landmarkIds().size())
                ...
                QLandmarkId qtLmId = fetchRequest->landmarkIds().at(
                        aData->iOpCount);
        ...
        case QLandmarkAbstractRequest::CategoryFetchByIdRequest:
            ...
            if (aData->iOpCount < fetchRequest->categoryIds().size())
                ...
                QLandmarkCategoryId qtCatId = fetchRequest->categoryIds().at(
                        aData->iOpCount);
                QLandmarkCategory fetchedQtCategory = category(qtCatId,
                        &error, &errorString);
        ...
        case QLandmarkAbstractRequest::LandmarkFetchRequest:
            ...
            if (aData->iOpCount > 0)
                ...
                if (aData->iLandmarkSearch)
                    ...
                    result = LandmarkUtility::convertToQtLandmarkIds(
                            managerUri(), symbianLmIds);
                    symbianLmIds.Close();
                    }
                ...
                sortFetchedLmIds(limit, offset, sortOrders, result,
                        filters.at(aData->iOpCount - 1).type(), &error,
                        &errorString);
                ...
                if (aData->iOpCount == 1)
                    ...
                    for (int j = 0; j < aData->iLandmarkIds.size(); ++j)
                        {
                        if (aData->iLandmarkIds.at(j).isValid())
                            aData->iLocalIds.insert(
                                    aData->iLandmarkIds.at(j).localId());
                        }
                    } // if aData->iOpCount = 1
            ...
            CPosLmSearchCriteria* searchCriteria = getSearchCriteriaL(
                    filters.at(aData->iOpCount));
            ...
            if (sortOrders.isEmpty())
                lmOperation = getOperationL(aData->iLandmarkSearch,
                        searchCriteria, QLandmarkSortOrder(), maxMatches);
            else
                lmOperation = getOperationL(aData->iLandmarkSearch,
                        searchCriteria, sortOrders.at(0), maxMatches);
 ...
 */
bool LandmarkManagerEngineSymbianPrivate::sortFetchedLmIds(int limit,
        int offset, QList<QLandmarkSortOrder> sortOrders,
        QList<QLandmarkId>& landmarkIds,
        QLandmarkFilter::FilterType filterType,
        QLandmarkManager::Error *error, QString *errorString) const
    {
    ...
    if (offset >= landmarkIds.size())
        ...
        *error = QLandmarkManager::DoesNotExistError;
        *errorString = QString(
                "No landmarks found, from provided offset value.");
        }
    ...
    if (sortRequired)
        ...
        landmarkIds.clear();
        landmarkIds = QLandmarkManagerEngineSymbian::sortLandmarks(landmarks,
                sortOrders);
        }
    ...
    if ((filterType == QLandmarkFilter::IntersectionFilter || filterType
        ...
        landmarkIds = this->landmarkIds(tmpFilter, KAllLandmarks,
                KDefaultIndex, sortOrders, error, errorString);
        }

    qDebug() << "result size = " << resultcount << " limit = " << limit
            << " offset = " << offset;
    return true;
    }
...
void LandmarkManagerEngineSymbianPrivate::exportLandmarksL(QIODevice *device,
    ...
    if (!landmarkIds.size())
        ...
        foreach(const QLandmarkId& id,landmarkIds)
                selectedLandmarks.AppendL(
                        LandmarkUtility::convertToSymbianLandmarkId(id));
        }
    ...
    if (outputdevice)
        ...
        // Performs export of landmarks  and cleanup of CPosLmOperation
        ExecuteAndDeleteLD(m_LandmarkDb->ExportLandmarksL(*encoder,
                selectedLandmarks, transferOption));
 ...
 * import landmarks
 */
QList<QLandmarkId> LandmarkManagerEngineSymbianPrivate::importLandmarksL(
        QIODevice *device, const QString &format,
        QLandmarkManager::TransferOption option,
    ...
    if (outputdevice)
        ...
        RFile fileName;
        TInt
                err(fileName.Open(fs, pathPtr, EFileShareReadersOnly
                        | EFileRead));
        ...
        //If package format is not supported leave
        if (err == KErrNone && (dataType.iConfidence
                == CApaDataRecognizerType::EProbable || dataType.iConfidence
                == CApaDataRecognizerType::ECertain || dataType.iConfidence
                == CApaDataRecognizerType::EPossible))
            {
        ...
        if (outputdevice)
            ...
            //
            HBufC8* packageFormat =
                    LandmarkUtility::landmarkPackageFormatsStrL(format);
            CleanupStack::PushL(packageFormat);
            ...
            CPosLmItemIterator* iter =
                    m_LandmarkDb->ImportedLandmarksIteratorL(op);
            CleanupStack::PushL(iter);
    ...
    if (addSinglecategoryrequired)
        {
        ExecuteAndDeleteLD(m_LandmarkCatMgr->AddCategoryToLandmarksL(catId,
                landmarkIds));
        }

    importedLmIds = LandmarkUtility::convertToQtLandmarkIds(managerUri(),
            landmarkIds);
 ...
 */
void LandmarkManagerEngineSymbianPrivate::handleDatabaseEvent(
        const TPosLmEvent& aEvent)
    {
    ...
    switch (aEvent.iEventType)
        ...
        case EPosLmEventLandmarkCreated:
            ...
            if (m_CreatedLmIds.contains(localId, Qt::CaseInsensitive))
                ...
                lmids << lmid;
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::landmarkAdded, lmids);
                }
        ...
        case EPosLmEventLandmarkDeleted:
            ...
            if (m_DeletedLmIds.contains(localId, Qt::CaseInsensitive))
                ...
                lmids << lmid;
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::landmarkRemoved, lmids);
                }
        ...
        case EPosLmEventLandmarkUpdated:
            ...
            if (m_UpdatedLmIds.contains(localId, Qt::CaseInsensitive))
                ...
                lmids << lmid;
                m_LmEventObserver.handleLandmarkEvent(
                        LandmarkEventObserver::landmarkUpdated, lmids);
                }
        ...
        default:
            {
            m_LmEventObserver.handleLandmarkEvent(
                    LandmarkEventObserver::unknownChanges);
            }
