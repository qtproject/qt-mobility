...
CECntGetDefinitionsForExistingView1Step::CECntGetDefinitionsForExistingView1Step()
/** Each test step initialises it's own name
*/
	{
	// store the name of this test case
	// this is the name that is used by the script file
	//DEF iTestStepName = _L("CECntGetDefinitionsForExistingView1Step");
...
*/
TInt CECntGetDefinitionsForExistingView1Step::Exec_SendReceive()
	{
	...
	if(iResult_Server == 0)
		...
		if(SR_MESSAGE_ID >= 0)
			...
			TPtr ptr = buf->Des();
			HBufC8 *buf8 = NULL;
			TRAP(err, buf8 = HBufC8::NewL(1000) );
			TEST(err == KErrNone);
			TPtr8 ptr8 = buf8->Des();
			#if defined(_DEBUG)	
			iResult_SR	=	SendReceive(SR_MESSAGE_ID,TIpcArgs(&ptr,&ptr8,&ptr,&ptr));
			#else
			iResult_SR = KErrNone;
			#endif
			delete buf8 ;
