...
QMessage TestSms::createEmail(const QString &text)
{
    ...
    if (m_pMM)
    {
        QMessageAccountIdList accountsIdList = m_pMM->queryAccounts();
        int count = accountsIdList.count();
        qDebug() << "accounts_counter = "  << count;

        for (int i = 0; i < count; i++)
        {
            QMessageAccountId id = accountsIdList[i];
            QMessageAccount l_account = m_pMM->account(id);
            if (l_account.messageTypes() & QMessage::Email) {
...
bool TestSms::startEmailSending()
{
    bool ret = false;

    QMessage m = createEmail("E-mail sending test");

    qDebug() << "Parent account ID = " << m.parentAccountId().toString();

    ret = m_pMS && m_pMS->send(m);

    if (ret)
    {
        qDebug() << "startEmailSending(): successfull EMail sending  " << endl;
    }
    else
    {
        qDebug() << "startEmailSending(): EMail sending failed " << endl;
    }
...
void TestSms::construct()
{
    QMessageManager *mm = new QMessageManager();
    QMessageService *ms = new QMessageService();

    qDebug() << "CONSTRUCTED!!!!";
}

void TestSms::testCountAccounts()
{
    if (m_pMM)
    {
        int accounts_counter = m_pMM->countAccounts();
        qDebug() << "accounts_counter = "  << accounts_counter;
    }
    else
        qDebug() << "Message Manager is not available";
}

void TestSms::testQueryAccounts()
{
    if (m_pMM)
    {
        QMessageAccountIdList accountsIdList = m_pMM->queryAccounts(/*QMessageAccountFilter::byName("work")*/);
        int count = accountsIdList.count();
        qDebug() << "accounts_counter = "  << count;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            qDebug() << "======================================";
            qDebug() << "id = " << id.toString();
            qDebug() << "account.name = " << l_account.name();
            qDebug() << "======================================";
        }
...
bool TestSms::testRemoveQMFAccount(const QString & accountIdStr)
{
    bool result = false;
    QMessageAccountId aId(accountIdStr);

    if (aId.isValid() && m_pMM)  {
        result = m_pMM->removeAccount(aId);
    }
