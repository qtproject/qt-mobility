...
void tst_QContactManager::signalEmission()
{
    ...
    if (m1->information()->hasFeature(QContactManagerInfo::Groups)) {
        ...
        adder->removeContact(c.localId());
        adder->removeContact(c2.localId());
        ...
        QVERIFY(sigids.value(0) == g1.localId());

        // Add one member (via the group)
        g1.addMember(c.localId());
        QVERIFY(adder->saveGroup(&g1));
        ...
        sigids = args.at(0).value<QList<QContactLocalId> >();
        QVERIFY(sigids.count() == 1);
        QVERIFY(sigids.value(0) == c.localId());
        ...
        c2.setGroups(QList<QContactLocalId>() << g1.localId());
        ...
        sigids = args.at(0).value<QList<QContactLocalId> >();
        QVERIFY(sigids.count() == 1);
        QVERIFY(sigids.value(0) == c2.localId());
        ...
        // Delete the group
        QVERIFY(adder->removeGroup(g1.localId()));
        QTRY_VERIFY(spyCA.count() == 0);
        QTRY_VERIFY(spyCM.count() == 1);
        QTRY_VERIFY(spyCR.count() == 0);
        QTRY_VERIFY(spyGA.count() == 0);
