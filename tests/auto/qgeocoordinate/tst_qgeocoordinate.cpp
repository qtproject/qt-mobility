...
private slots:
    ...
    void distanceTo_data()
    {
        ...
        QTest::newRow("brisbane -> melbourne")
                << BRISBANE << MELBOURNE << 1374820.1618767744;
        QTest::newRow("london -> new york")
                << LONDON << NEW_YORK << 5570538.4987236429;
        QTest::newRow("north pole -> south pole")
                << NORTH_POLE << SOUTH_POLE << 20015109.4154876769;
    }
    ...
    void azimuthTo_data()
    {
        ...
        QTest::newRow("invalid coord 2")
                << BRISBANE << QGeoCoordinate() << 0.0;
        QTest::newRow("brisbane -> melbourne")
                << BRISBANE << MELBOURNE << 211.1717286649;
        QTest::newRow("london -> new york")
                << LONDON << NEW_YORK << 288.3388804508;
        QTest::newRow("north pole -> south pole")
