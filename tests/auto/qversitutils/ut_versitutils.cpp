...
void UT_VersitUtils::testExtractVCard21PropertyParams()
{
    ...
    // Test wide character support.
    QTextCodec* codec = QTextCodec::codecForName("UTF-16BE");
    // Remove the byte-order mark generated by codec.
    QByteArray data = codec->fromUnicode(QString::fromAscii(";HOME;CHARSET=UTF-16:123")).mid(2);
    cursor.setData(data);
    cursor.setSelection(cursor.data.size());
...
void UT_VersitUtils::testExtractVCard30PropertyParams()
{
    ...
    // Test wide character support.
    QTextCodec* codec = QTextCodec::codecForName("UTF-16BE");
    QByteArray data = codec->fromUnicode(QString::fromAscii(";TIPE=HOME,VOICE;CHARSET=UTF-16:123"))
                      .mid(2); // Remove the byte-order mark generated by codec.
    cursor.setData(data);
    cursor.setSelection(cursor.data.size());
