...
void tst_QContactAsync::relationshipSave()
{
    ...
    // cancelling
    testRel.appendDestinationContact(aId); // shouldn't get saved (circular anyway)
    saveList.clear();
    saveList << testRel;
    rsr.setRelationships(saveList);
    QCOMPARE(rsr.relationships(), saveList);
