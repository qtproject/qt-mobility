...
void TestQGeoPositionInfoSource::lastKnownPosition_data()
{
    ...
    if (m_source != 0) {
        PositioningMethods methods = m_source->supportedPositioningMethods();
        if ((methods & SatellitePositioningMethods) != 0) {
            QTest::newRow("satellite - false")
            << int(SatellitePositioningMethods)
            << false << true;
            QTest::newRow("satellite - true")
            << int(SatellitePositioningMethods)
            << true << true;
        }
        if ((methods & NonSatellitePositioningMethods) != 0) {
            QTest::newRow("nonsatellite - false")
            << int(NonSatellitePositioningMethods)
            << false << false;
            QTest::newRow("nonsatellite - true")
            << int(NonSatellitePositioningMethods)
            << true << true;
        }
        if ((methods & AllPositioningMethods) != 0) {
            QTest::newRow("all - false")
            << int(AllPositioningMethods)
            << false << true;
