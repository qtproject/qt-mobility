...
void tst_qnearfieldtagtype4::testCommandSet()
{
    ...
    // read cc
    QNearFieldTarget::RequestId id3 = tester.target->read(0x08,0x07);
    ...
    ++okCount;
    QVERIFY(readResp.at(readResp.count()-1) == 0x00);
    QVERIFY(readResp.at(readResp.count()-2) == 0x90);
    ...
    // read NDEF
    QNearFieldTarget::RequestId id5 = tester.target->read(0x00,0x02);
    QVERIFY(tester.target->waitForRequestCompleted(id5));
    ++okCount;
    QByteArray readNdefResp = tester.target->requestResponse(id5).toByteArray(); 
    ++okCount;
    QVERIFY(readNdefResp.at(readNdefResp.count()-1) == 0x00);
    QVERIFY(readNdefResp.at(readNdefResp.count()-2) == 0x90);
    ...
    // write NDEF
    QNearFieldTarget::RequestId id6 = tester.target->write(data, 0x00);
    QVERIFY(tester.target->waitForRequestCompleted(id6));
    ++okCount;
    QCOMPARE(tester.target->requestResponse(id6).toByteArray(), resp);
    
    // read again
    QNearFieldTarget::RequestId id7 = tester.target->read(0x00,0x02);
