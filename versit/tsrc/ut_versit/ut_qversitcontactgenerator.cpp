...
void UT_QVersitContactGenerator::testOrganization()
{
    ...
    foreach(const QContactDetail& d,details){
        ...
        if(!o.name().isEmpty()){
            expectedFileName = o.name() + "_" + o.department();
        }else if(!o.logo().isEmpty()){
            QString fileName = o.logo();            
            QVERIFY(fileName.contains(expectedFileName));
            QVERIFY(fileName.endsWith(QString::fromAscii(versitPhotoGif).toLower()));
            QFile file(fileName);
...
"ejqfsruRgSSMw9x0B/SuTLk/aP0NKux//9k=";
    ...
    QVERIFY(fileName.endsWith(QString::fromAscii(versitPhotoJpeg).toLower()));
    QFile file(fileName);
    QVERIFY(file.open(QIODevice::ReadOnly));
    QByteArray content = file.readAll();
    file.close();
...
void UT_QVersitContactGenerator::testAvatarJpegTwoContactsWithSameName()
{
    ...
    QContactAvatar avatar1 =
            static_cast<QContactAvatar>(
                    contact1.detail(QContactAvatar::DefinitionName));
    QContactAvatar avatar2 =
            static_cast<QContactAvatar>(
                    contact2.detail(QContactAvatar::DefinitionName));
...
void UT_QVersitContactGenerator::testSound()
{
    ...
    QVersitProperty nameProperty;
    QStringList valName;
    valName.append("Homer");//FirstName
    valName.append("Simpson");//LastName
    nameProperty.setName(QString::fromAscii(versitNameId));
...
QVersitDocument UT_QVersitContactGenerator::createDocumentWithNameAndPhoto(
        const QByteArray& name, QByteArray image,
        const QString& imageType, const QString& encoding)
{
    ...
    QVersitProperty property;
    property.setName(QString::fromAscii(versitPhotoId));
    if(encoding == versitEncodingBase64){
